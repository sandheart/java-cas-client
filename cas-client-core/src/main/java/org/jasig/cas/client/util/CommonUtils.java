...
public final class CommonUtils {
    ...
    public static String handRedirUrl(HttpServletRequest request, final String potentialRedirectUrl) {
        String reqUrl = request.getRequestURL().toString();
        Matcher matcher = Pattern.compile("//(.*?)/").matcher(reqUrl);
        String host = "";
        if (matcher.find()) {
        ...
        if (host != null && host.length() > 0) {
            String redirUrl = potentialRedirectUrl;
            return redirUrl.replaceAll("%2F%2F.*%2F", "%2F%2F" + host + "%2F");
        }
        return potentialRedirectUrl;
    }
